//! RepairContext — prefetched HTML only; no MCP / port handles (§14.4).
...
impl RepairContext {
    ...
    /// Convenience for tests / adapters: fixed family, empty HTML.
    pub fn new(source_key: SourceKey, source: BookSource, family: SiteFamily) -> Self {
        Self::builder(source_key, source).family(family).build()
    }

    pub fn with_html(mut self, url: impl AsRef<str>, body: impl Into<Vec<u8>>) -> Self {
        if let Ok(u) = Url::new(url.as_ref()) {
            self.html.insert(u, body.into());
        }
        self
    }
    ...
    pub fn primary_url(&self) -> Result<Url, TypeError> {
